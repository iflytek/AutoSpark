...
export const formatTimeDifference = (timeDifference) => {
  ...
  for (let i = 0; i < units.length; i++) {
    ...
    if (timeDifference[unit] !== 0) {
      if (unit === 'minutes') {
        return `${timeDifference[unit]}分钟之前`;
      } else {
        return `${timeDifference[unit]} ${unit}之前`;
      }
    }
  }
...
export const returnToolkitIcon = (toolkitName) => {
  const toolkitData = [
    ...
    {name: '代码工具', imageSrc: '/images/spark_icon.png'},
    ...
    {name: 'docker tools', imageSrc: '/images/artifactory.png'},
    {name: 'Intelligent customer service Toolkit', imageSrc: '/images/智能客服.svg'},
    {name: 'Book Food Toolkit', imageSrc: '/images/园区外卖.svg'},
    {name: 'Model Train Toolkit', imageSrc: '/images/atp.png'},
