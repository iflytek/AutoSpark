...
export default function AgentSchedule({internalId, closeCreateModal, type, agentId, setCreateModal, setCreateEditModal, env}) {
  ...
  const [modalHeading, setModalHeading] = useState('安排运行')
  const [modalButton, setModalButton] = useState('创建并安排运行')
  ...
  const addScheduledAgent = () => {
    if ((startTime === '' || (isRecurring === true && (timeValue == null || (expiryType === "After certain number of runs" && (parseInt(expiryRuns, 10) < 1)) || (expiryType === "Specific date" && expiryDate == null))))) {
      toast.error('请输入正确的详细信息', {autoClose: 1800});
      return;
    }
    ...
    <div>
      <div className="modal" onClick={closeCreateModal}>
        <div className="modal-content" style={{width: '35%'}} onClick={preventDefault}>
          ...
          <div>
            <label className={styles.form_label}>选择日期和时间</label>
            <div>
          ...
          {isRecurring && (<div style={{marginTop: '20px'}}>
            <div style={{ marginBottom: '10px'}}>重复运行详细信息</div>
            <label className={styles.form_label}>重复每个任务</label>
            ...
            <label className={styles.form_label}>停止策略</label>
            <div>
