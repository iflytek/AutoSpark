...
from autospark.controllers.types.agent_execution_config import AgentRunIn
...
class AgentConfiguration(DBBaseModel):
    ...
    @classmethod
    def get_model_api_secret(cls, session, agent_id: int, model: str):
        """
        Get the model API secret from the agent id.
        ...
        Returns:
            str: The model API secret.
        """
        config_model_source = Configuration.fetch_value_by_agent_id(session, agent_id,
                                                                    "model_source") or "SparkAI"
        selected_model_source = ModelSourceType.get_model_source_from_model(model)
        if selected_model_source.value == config_model_source:
            config_value = Configuration.fetch_value_by_agent_id(session, agent_id, "model_api_secret")
            model_api_key = decrypt_data(config_value)
            return model_api_key
    ...
    @classmethod
    def get_model_app_id(cls, session, agent_id: int, model: str):
        """
        Get the model API key from the agent id.
        ...
        selected_model_source = ModelSourceType.get_model_source_from_model(model)
        if selected_model_source.value == config_model_source:
            config_value = Configuration.fetch_value_by_agent_id(session, agent_id, "model_app_id")
