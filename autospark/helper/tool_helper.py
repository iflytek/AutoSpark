...
from autospark_kit.tools.base_tool import BaseToolkit
...
def download_tool(tool_url, target_folder):
    ...
    logger.info("DownLoad tool Success!")
    os.remove(tool_zip_file_path)
    write_metadata(target_folder, {"code_link": tool_url, "author": owner})


def write_metadata(target_folder, metadata: dict):
    if os.path.isdir(target_folder):
        fi = os.path.join(target_folder, 'metadata.json')
        f = open(fi, 'w')
        json.dump(metadata, f)
        f.close()
        logger.info("Write Metadata.json Success!")
...
def init_tools(folder_path, session, tool_name_to_toolkit):
    ...
    for folder_name in os.listdir(folder_path):
        ...
        if os.path.isdir(folder_dir):
            # sys.path.append(os.path.abspath('autospark/tools/email'))
            sys.path.append(folder_dir)
            for file_name in os.listdir(folder_dir):
                file_path = os.path.join(folder_dir, file_name)
                if file_name.endswith(".py") and not file_name.startswith("__init__"):
                    # Get classes
                    classes = get_classes_in_file(file_path=file_path, clazz=BT)
...
def init_toolkits(code_link, existing_toolkits, folder_path, organisation, session):
    ...
    for folder_name in os.listdir(folder_path):
        ...
        author = "internal"
        if os.path.isdir(folder_dir):
            if code_link is None and os.path.exists(os.path.join(folder_dir, "metadata.json")):
                meta = json.load(open(os.path.join(folder_dir, "metadata.json"), 'r'))
            ...
            for file_name in os.listdir(folder_dir):
                ...
                if file_name.endswith(".py") and not file_name.startswith("__init__"):
                    # Get classes
                    classes = get_classes_in_file(file_path=file_path, clazz=BTK)
