...
from autospark.helper.tool_helper import handle_tools_import
...
app = Celery("autospark", include=["autospark.worker"], imports=["autospark.worker"])
...
def execute_agent(agent_execution_id: int, time):
    ...
    from autospark.jobs.agent_executor import AgentExecutor
    handle_tools_import()
...
def summarize_resource(agent_id: int, resource_id: int):
    ...
    from autospark.resource_manager.resource_manager import ResourceManager
