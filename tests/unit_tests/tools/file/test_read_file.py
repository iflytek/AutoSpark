...
def test_read_file_success(read_file_tool):
    ...
    with patch('builtins.open', mock_file), \
            ...
            patch('autospark.helper.resource_helper.ResourceHelper.get_root_output_dir',
                  return_value="/output_dir/{agent_id}/"), \
            patch('autospark.models.agent.Agent.get_agent_from_id', return_value=Agent(id=1, name='TestAgent')), \
            patch('autospark.models.agent_execution.AgentExecution.get_agent_execution_from_id',
                  return_value=
