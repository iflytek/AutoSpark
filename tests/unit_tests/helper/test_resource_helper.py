...
def test_make_written_file_resource(mocker):
    ...
    mocker.patch('autospark.helper.resource_helper.get_config', side_effect=['FILE','/','/','FILE'])
    mock_agent = Agent(id=1, name='TestAgent')
    mock_agent_execution = AgentExecution(id=1, name='TestExecution')

    with patch('autospark.helper.resource_helper.logger') as logger_mock:
...
def test_get_resource_path(mocker):
    mocker.patch('os.getcwd', return_value='/')
    mocker.patch('autospark.helper.resource_helper.get_config', side_effect=['/'])
